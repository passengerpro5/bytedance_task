# Script Agent Edit Output Contract

Return valid JSON only with exactly one revised script item object. Do not wrap the result in a `scripts` array.

The object must contain the same inner fields as one item inside the generation contract:
...
- Preserve source IDs and current persona continuity unless the edit instruction explicitly asks to change them.
